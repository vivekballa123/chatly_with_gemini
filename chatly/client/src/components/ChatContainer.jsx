...
const ChatContainer = () => {
    ...
    const handleSendMessage = async (e) => {
        ...
        if(input.trim().toLowerCase().startsWith('@ai ')){
            const query = input.trim().slice(4).trim()
            if(query){
                setAiQuery(query)
                setShowAiPanel(true)
                setInput("")
  ...
  return selectedUser ? (
      ...
      <div className='flex flex-col h-[calc(100%-120px)] overflow-y-scroll p-3 pb-6'>
       ...
       {/*------BOTTOM AREA ------- */}         
        <div className='absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3'>
            <div className='flex-1 flex items-center bg-gray-100/12 px rounded-full '>
                <input onChange={(e)=>setInput(e.target.value)} value={input} onKeyDown={(e)=>e.key === "Enter" ? handleSendMessage(e):null} type="text" placeholder='Message or "@ai your question"' name="" id="" className='flex-1 text-sm p-3 border-none rounded-lg outline-none text-white placeholder-gray-400' />
    ...
    <div className='flex flex-col items-center justify-center gap-2 text-gray-500 bg-white/10 max-md:hidden'>
        ...
        <p className='rtext-lg font-medium text-white'> Chat anytime,anywhere</p>
    </div>
  )
}
